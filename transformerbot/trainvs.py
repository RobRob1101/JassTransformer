...
import numpy as np
from rulebasedbot.bot import RuleBasedBot
import os
...
def get_card_from_id(card_id):
    ...
    try:
        color_idx = card_id // 9
        number = (card_id % 9) + 6
        return {"color": colors[color_idx], "number": number}
    except Exception:
        return None
...
class VSTrainer:
    def __init__(self):
        ...
        self.opponent = RuleBasedBot()

        
    def collect_trajectories(self, num_games=10):
        self.model.eval()
        
        opponent_model = self.opponent
        ...
        for _ in range(num_games):
            ...
            while not done:
                ...
                if current_p % 2 == 0:
                    ...
                    with torch.no_grad():
                        logits, value = self.model(c, p, t_idx, turn, m, mask)
                    ...
                    player_transitions[current_p]["actions"].append(action)
                    player_transitions[current_p]["values"].append(value.item())
                    player_transitions[current_p]["log_probs"].append(log_prob.item())

                else:
                    valid_actions = env.get_legal_actions(current_p)
                    valid_cards_full = [get_card_from_id(c) for c in valid_actions]
                    ...
                    if env.mode == 0:
                        ...
                        opp_mode = "TRUMPF"
                        opp_trump = ["SPADES", "HEARTS", "DIAMONDS", "CLUBS"][env.mode - 3]

                    self.opponent.mode = opp_mode
    ...
    def train(self, iterations=1000):
        ...
        for it in range(iterations):
            trajectories = self.collect_trajectories(num_games=10) # Adjust batch size based on RAM
            if not trajectories:
                continue
            ...
            print(f"Iteration {it+1}/{iterations} | Trajectories: {len(trajectories)} | Avg Return: {avg_return:.2f}")
